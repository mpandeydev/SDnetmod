...
from Models.Layers import MaxPooling, CNN, dropout, RNN_from_opt, set_dropout_prob, weighted_avg, set_seq_dropout, \
    Attention, DeepAttention, LinearSelfAttn, GetFinalScores
...
class SDNet(nn.Module):
    def __init__(self, opt, word_embedding):
        ...
        self.context_rnn, context_rnn_output_size = RNN_from_opt(x_input_size, opt['hidden_size'],
                                                                 ...
                                                                 concat_rnn=opt['concat_rnn'], add_feat=addtional_feat)
        ...
        print('After Input LSTM, the vector_sizes [doc, query] are [', context_rnn_output_size, ques_rnn_output_size,
              '] *', opt['in_rnn_layers'])
        ...
        self.deep_attn = DeepAttention(opt, abstr_list_cnt=opt['in_rnn_layers'],
                                       ...
                                       correlation_func=3, word_hidden_size=vocab_dim + addtional_feat)
    ...
    def forward(self, x, x_single_mask, x_char, x_char_mask, x_features, x_pos, x_ent, x_bert, x_bert_mask,
                x_bert_offsets, q, q_mask, q_char, q_char_mask, q_bert, q_bert_mask, q_bert_offsets, context_len):
        '''
        ...
        x_input_list = [
            dropout(x_word_embed, p=self.opt['dropout_emb'], training=self.drop_emb)]  # batch x x_len x vocab_dim
        ques_input_list = [
            dropout(ques_word_embed, p=self.opt['dropout_emb'], training=self.drop_emb)]  # batch x q_len x vocab_dim
        ...
        x_input_list.append(
            x_features)  # batch x x_len x (vocab_dim + cdim + vocab_dim + pos_dim + ent_dim + feature_dim)
        ...
        # Multi-layer RNN
        _, x_rnn_layers = self.context_rnn(x_input, x_mask, return_list=True,
                                           x_additional=x_cemb)  # layer x batch x x_len x context_rnn_output_size
        _, ques_rnn_layers = self.ques_rnn(ques_input, q_mask, return_list=True,
                                           x_additional=ques_cemb)  # layer x batch x q_len x ques_rnn_output_size

        # rnn with question only 
        ques_highlvl = self.high_lvl_ques_rnn(torch.cat(ques_rnn_layers, 2),
        ...
        x_rnn_after_inter_attn, x_inter_attn = self.deep_attn([x_long], x_rnn_layers, [ques_long], ques_rnn_layers,
                                                              x_mask, q_mask, return_bef_rnn=True)
    ...
    def character_cnn(self, x_char, x_char_mask):
        ...
        x_char_cnn = self.char_cnn(x_char_embed.contiguous().view(-1, char_num, char_dim),
                                   x_char_mask)  # (batch x word_num) x char_num x char_cnn_hidden_size
        x_char_cnn_final = self.maxpooling(x_char_cnn, x_char_mask.contiguous().view(-1, char_num)).contiguous().view(
            batch_size, word_num, -1)  # batch x word_num x char_cnn_hidden_size

...
class StackedBRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, rnn_type=nn.LSTM, concat_layers=False, bidirectional=True,
                 add_feat=0):
        ...
        for i in range(num_layers):
            in_size = input_size if i == 0 else (
                self.bidir_coef * hidden_size + add_feat if i == 1 else self.bidir_coef * hidden_size)
...
def RNN_from_opt(input_size_, hidden_size_, num_layers=1, concat_rnn=False, add_feat=0, bidirectional=True,
                 rnn_type=nn.LSTM):
    new_rnn = StackedBRNN(
...
class GetFinalScores(nn.Module):
    def __init__(self, x_size, h_size):
        """
        :param x_size: context_final_size
        :param h_size: ques_final_size
        """
    ...
    def forward(self, x, h0, x_mask):
        """
        x = batch * len * x_size: x_final
        h0 = batch * h_size: ques_merged
        x_mask = batch * len: x_mask
        """
    ...
    def get_single_score(self, x, h, x_mask, linear, w):
        """
        :param x: batch * len * x_size: x_final
        :param h: batch * h_size
        :param x_mask: batch * len: x_mask
        :param linear:
        :param w:
        :return:
        """
    ...
    def __init__(self, opt, abstr_list_cnt, deep_att_hidden_size_per_abstr, correlation_func=1, word_hidden_size=None):
        ...
        for i in range(abstr_list_cnt + 1):
            self.int_attn_list.append(
                Attention(att_size, deep_att_hidden_size_per_abstr, correlation_func=correlation_func))

...
class SDNetTrainer(BaseTrainer):
    ...
    def official(self, model_path, test_data):
        ...
        test_batches = BatchGen(self.opt, test_data['data'], self.use_cuda, self.preproc.train_vocab,
                                self.preproc.train_char_vocab, evaluation=True)
        predictions = []
        confidence = []
        final_json = []
        cnt = 0
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_start, numEpochs):
            ...
            dev_batches = BatchGen(self.opt, dev_data['data'], self.use_cuda, self.vocab, self.char_vocab,
                                   evaluation=True)
            for i, batch in enumerate(train_batches):
                if i == len(train_batches) - 1 or (epoch == 0 and i == 0 and ('RESUME' in self.opt)) or (
                        i > 0 and i % 1500 == 0):
    ...
    def update(self, batch):
        ...
        score_s, score_e, score_yes, score_no, score_no_answer = self.network(x, x_mask, x_char, x_char_mask,
                                                                              x_features, x_pos, x_ent, x_bert,
                                                                              x_bert_mask, x_bert_offsets,
                                                                              query, query_mask, query_char,
                                                                              query_char_mask, query_bert,
